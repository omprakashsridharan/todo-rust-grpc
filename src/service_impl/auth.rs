...
use crate::service::auth::auth_server::{Auth, AuthServer};
...
impl Auth for AuthService {
    async fn sign_up(&self, request: Request<SignUpRequest>) -> Result<Response<SignUpResponse>, Status> {
        ...
        if request.get_ref().pin > 9999 || request.get_ref().pin < 1000 {
            let error_message = format!("Sign Up for Username: {} - PIN should consist only 4 digits", request.get_ref().username.to_string());
            error!("{}",error_message);
